    ...
    while True:
        try:
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                # Build ONE RRset for this (name, class, type) triple, add every
                # rdata to it, then append it once to the answer section.
                # Creating a fresh RRset per-rdata (old approach) left each RRset
                # with only one record and no TTL, causing dnspython's wire encoder
                # to silently omit the answer on certain record types.
                rrset = dns.rrset.RRset(question.name, dns.rdataclass.IN, qtype)
                rrset.update_ttl(300)   # 5-minute TTL; required for valid wire output
