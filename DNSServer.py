...
dns_records = {
    ...
    # ------------------------------------------------------------------
    'safebank.com.': {
        # Real safebank.com A record
        dns.rdatatype.A: '52.2.19.190',
    },
    ...
    # ------------------------------------------------------------------
    'nyu.edu.': {
        # Real NYU DNS values (verified from public DNS records / Wireshark lab)
        dns.rdatatype.A:    '216.165.47.12',
        # Real NYU IPv6 address
        dns.rdatatype.AAAA: '2607:f600:1002:6113::100',
        # Real NYU MX: Proofpoint mail gateway (priority 10)
        dns.rdatatype.MX:   [(10, 'mxa-00256a01.gslb.pphosted.com.')],
        # Real NYU authoritative nameserver
        dns.rdatatype.NS:   'ns1.nyu.edu.',
        # Exfiltration payload: Fernet token stored as a QUOTED TXT string.
        # Quoting is required because the base64url token contains '=' chars
        # that dns.rdata.from_text() would misparse in an unquoted context.
        # dnspython strips the quotes on the wire and returns the raw bytes
        # via rrset[0].strings[0], giving back the exact token for decryption.
    ...
    while True:
        try:
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                # One RRset for this (name, class, type) – all rdata go into it.
                # Creating a new RRset per rdata leaves TTL=0 on each one,
                # causing dnspython's wire encoder to drop the answers.
