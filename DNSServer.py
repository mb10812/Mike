...
input_string = 'Exfiltrated secret data hidden in DNS TXT record!'
...
dns_records = {
    'example.com.': {
        ...
        # MX: list of (preference, mail-server) tuples – handled specially below
        dns.rdatatype.MX:   [(10, 'mail.example.com.')],
        dns.rdatatype.CNAME: 'www.example.com.',
        dns.rdatatype.NS:   'ns.example.com.',
        dns.rdatatype.TXT:  ('This is a TXT record',),
        # SOA tuple order: mname, rname, serial, refresh, retry, expire, minimum
        dns.rdatatype.SOA:  (
            'ns1.example.com.',   # mname  – primary nameserver
            'admin.example.com.', # rname  – responsible mailbox (. replaces first @)
            2023081401,           # serial  – version stamp (YYYYMMDDnn)
            3600,                 # refresh – secondary poll interval (seconds)
            1800,                 # retry   – retry after failed refresh
            604800,               # expire  – drop zone if no contact for 7 days
            86400,                # minimum – negative-cache TTL (RFC 2308)
        ),
    },
    ...
    # EXFILTRATION RECORD
    # -----------------------------------------------------------------------
    # Storing the Fernet-encrypted ciphertext as a TXT record.
    # The value is already base64url bytes; decode to str for the DNS wire format.
    'secret.example.com.': {
        dns.rdatatype.A: '192.168.1.200',
        dns.rdatatype.TXT: (encrypted_value.decode('utf-8'),),
    },

    # Additional records to round out the zone
    'mail.example.com.': {
    ...
    while True:
        try:
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                # Attach each rdata object to its own RRset in the answer section
