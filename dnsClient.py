...
def dns_query(type, name, server):
    ...
    # Bit positions in the FLAGS word (16 bits):
    # QR=15, OPCODE=11-14, AA=10, TC=9, RD=8, RA=7, Z=4-6, RCODE=0-3
    ...
    qname_encoded_parts = [struct.pack('B', len(part)) + part.encode('ascii') for part in qname_parts]  # ASCII encoding
    qname_encoded = b''.join(qname_encoded_parts) + b'\x00'  # Null byte terminates the domain name

    # Encode the QTYPE and QCLASS

    if type == 'A':
        qtype = 1      # A record = 1 (RFC 1035)
    elif type == 'AAAA':
        qtype = 28     # AAAA record = 28 (RFC 3596)
    else:
    ...
    data, _ = sock.recvfrom(4096)  # 4096 byte buffer
    ...
    response_question = data[12:12+len(question)]  # Start after 12-byte header
    assert response_question == question

    # Parse the response answer section
    response_answer = data[12+len(question):]  # Start after header + question
    offset = 0
    for _ in range(ANCOUNT):
        ...
        offset += 10  # Advance past the 10 bytes just parsed
